...
class SCECLRBase(nn.Module):
    ...
    def update_s(self, qii, qij, qji, feats_idx):
        ...
        self.xi = self.xi + torch.sum((1 - self.alpha) * torch.sum(q) )
...
class CosineLoss(SCECLRBase):
    ...
    def forward(self, feats, feats_idx):
        ...
        # Attraction
        feats_idx_double = torch.cat([feats_idx, feats_idx], dim=0)
        s_inv_hat = self.s_inv[feats_idx_double].unsqueeze(1) / self.N.pow(1)
        moment = 0.9

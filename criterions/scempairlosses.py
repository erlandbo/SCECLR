...
class CauchyLoss(SCEBase):
    ...
    def forward(self, y):
        ...
        pairdist = torch.cdist(y, y, p=2)

        q = 1.0 / ( pairdist.pow(2) + 1.0 )
        self_mask = torch.eye(2*B, dtype=torch.bool, device=y.device)
        pos_mask = torch.roll(self_mask, B, dims=1)

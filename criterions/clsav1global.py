...
class CLSABase(nn.Module):
    ...
    def update_s(self, qii, qij):
        ...
        # Repulsion
        Bij = qij.shape[0]
        self.xi = self.xi + torch.sum( (1 - self.alpha) * qij.detach().sum(dim=1) / (Bij - 1) )  # remove self-mask
...
class CauchyLoss(CLSABase):
    ...
    def forward(self, feats):
        ...
        repulsive_forces = torch.sum(q / Z_hat, dim=1) / (2 * B - 1)
...
class CosineLoss(CLSABase):
    ...
    def forward(self, feats):
        ...
        # Repulsion
        Z_hat = self.s_inv / self.N.pow(2)
        repulsive_forces = torch.sum(q / Z_hat, dim=1) / (2 * B - 1)  # (2B,)

...
# CLSAM
class CLSAv2NoPosLoss(nn.Module):
...
class CLSABase(nn.Module):
    ...
    def update_s(self, qii, qij, qji, feats_idx):
        ...
        # Repulsion
        E_rep1 = torch.sum(qij.detach(), dim=1) / (2.0 * B - 2.0)  # drop self-mask
        E_rep2 = torch.sum(qji.detach(), dim=1) / (2.0 * B - 2.0)  # drop self-mask
...
class CosineLoss(CLSABase):
    ...
    def forward(self, feats, feats_idx):
        ...
        Z_hat = ( self.s_inv[feats_idx] / self.N.pow(1) * ( 2 * B - 2) ).repeat(2)   # (B,) -> (2B,)

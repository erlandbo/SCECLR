...
class ResNet(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.resnet = torchvision.models.__dict__[resnet_arch](num_classes=num_classes, zero_init_residual=True)

        if not first_conv:
            self.resnet.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        if not first_maxpool:
            self.resnet.maxpool = nn.Identity()

    def forward(self, x):
        return self.resnet(x)  # (N, H)
...
class CLModel(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.mixer = mixer_encoder
        in_features = self.mixer.fc.weight.shape[1]
        self.mixer.fc = nn.Identity()

        self.projector = nn.Sequential(
            nn.Linear(in_features=in_features, out_features=mlp_hidden_features, bias=False),
            nn.BatchNorm1d(mlp_hidden_features) if norm_mlp_layer else nn.Identity(),
            nn.ReLU(),
            nn.Linear(in_features=mlp_hidden_features, out_features=mlp_outfeatures, bias=False)
        )

    def forward(self, x):
        hidden_feats = self.mixer(x)  # (N, H)
        latent_feats = self.projector(hidden_feats)  # (N, Z)

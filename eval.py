...
def evaluate(model, memory_loader, testloader):

    # Z_train, H_train, train_targets = data_features(model, memory_loader, device)
    # Z_test, H_test, test_targets = data_features(model, memory_loader, device)
    #
    ...
    # for nn in knns:
    #     for temp in temps:
    #         top1, top5 = test(model, memory_loader, testloader, k=nn, temperature=temp)
